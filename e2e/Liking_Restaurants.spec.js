...
Scenario('Searching Restaurants', async ({ I }) => {
  ...
  const matchingRestarants = names.filter((name) => name.indexOf(searchQuery) !== -1);

  I.fillField('#query', searchQuery);
  I.pressKey('Enter');

  const visibleLikedRestaurants = await I.grabNumberOfVisibleElements('.restaurant-item');
  assert.strictEqual(matchingRestarants.length, visibleLikedRestaurants);

  matchingRestarants.forEach(async (name, index) => {
